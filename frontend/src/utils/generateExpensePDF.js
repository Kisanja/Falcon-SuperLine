...
export const generateExpensePDF = (expenseData) => {
  ...
  doc.setTextColor(0);
  doc.setFont(undefined, 'bold');
  doc.text('Expenses Report', pageWidth / 2, contactY + 14, { align: 'center' });
  ...
  const totalExpense = expenseData.reduce((acc, item) => acc + Number(item.amount || 0), 0);
  ...
  // 🔸 Add Total Expense at Bottom
  const finalY = doc.lastAutoTable.finalY + 10;
  doc.setFontSize(12);
  doc.setFont(undefined, 'bold');
  doc.text(`Total Expense: Rs. ${totalExpense.toFixed(2)}`, pageWidth - 20, finalY, {
    align: 'right'
  });
